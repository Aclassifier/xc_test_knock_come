...
/** 
 * GENERATED BY GOOGLE AI TO FIND DROP_NEG_CNT_MAX
 * Iterates through the entire Xorshift32 period, prints statistics, 
 * and returns the longest continuous sequence of negative values.
 * 
 * @param initial_seed The starting state (must be non-zero).
 * @return The absolute maximum sequence of consecutive negative numbers.
 */
uint32_t find_max_negative_run(uint32_t initial_seed) { // Param 1 in test
    uint32_t state = initial_seed;
    uint32_t max_run = 0;
    uint32_t current_run = 0;
    uint64_t total_elements = 0;

    printf("Starting full state-space simulation. Please wait...\n");
    ...
    } while (state != initial_seed); // Terminates when we complete the full cycle
    ...
    printf("\n=== Simulation Complete ===\n");
    printf("Total numbers checked:  %llu (2^32 - 1)\n", (unsigned long long)total_elements);
